...
async function getJokeFromAPI() {
  const API_URL =
    "https://v2.jokeapi.dev/joke/Any?blacklistFlags=religious,political,racist,explicit";
  try {
    const response = await fetch(API_URL);
    const data = await response.json();

    // handle online jokes and two liners
...
// Tell Joke when button is clicked
